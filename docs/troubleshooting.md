...
Do not keep a signed copy and an unsigned copy around at the same time. They share the bundle identifier but not the signing identity, so granting permission to one revokes it for the other, and an ad-hoc build changes identity on every rebuild.

If building locally with `CODE_SIGNING_ALLOWED=NO` (or outside fastlane), macOS TCC will prompt in an endless loop unless the bundle is explicitly ad-hoc signed:

```bash
codesign --force --deep -s - --identifier "com.amethyst.Amethyst" /path/to/Amethyst.app
```
